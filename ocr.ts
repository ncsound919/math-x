...
import { z } from 'zod';
import { serverError, toLogMessage } from '../lib/errors';
...
router.post('/', async (req: Request, res: Response) => {
  let body: z.infer<typeof OCRSchema>;
  try {
    body = OCRSchema.parse(req.body);
  } catch (err) {
  ...
  try {
    ...
    console.error('OCR route error:', toLogMessage(err));
